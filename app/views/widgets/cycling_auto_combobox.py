# File: app/views/widgets/cycling_auto_combobox.py
...
class CyclingAutoCombobox(AutoCombobox):
	...
	def configure(self, cnf=None, **kw):
		...
		# When created, state is 'readonly'. Change to 'normal' to allow text entry for filtering.
		if 'state' in kw and kw['state'] == 'readonly':
			kw['state'] = 'normal'
		elif isinstance(cnf, dict) and 'state' in cnf and cnf['state'] == 'readonly':
			cnf['state'] = 'normal'
	...
	def __setitem__(self, key, value):
		if key == "values":
			self._invalidate_values_cache()
		if key == 'state' and value == 'readonly':
			value = 'normal'
