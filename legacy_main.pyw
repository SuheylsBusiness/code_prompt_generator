...
class TextEditorDialog(tk.Toplevel):
    ...
    def replace_stars(self):
        def clean_text(t):
            lines = unify_line_endings(t).split('\n')
            cleaned_lines = []
            for line in lines:
...
class HistorySelectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.current_page = 1
        self.items_per_page = tk.IntVar(value=10) # Default to 10 per page
    ...
    def create_pagination_controls(self):
        ...
        self.first_btn = ttk.Button(controls_frame, text="<< First", command=lambda: self.change_page('first'))
        self.first_btn.pack(side=tk.LEFT, padx=2)
        self.prev_btn = ttk.Button(controls_frame, text="< Prev", command=lambda: self.change_page('prev'))
        self.prev_btn.pack(side=tk.LEFT, padx=2)

        self.page_label = ttk.Label(controls_frame, text="Page 1 of 1")
        self.page_label.pack(side=tk.LEFT, padx=5)

        self.next_btn = ttk.Button(controls_frame, text="Next >", command=lambda: self.change_page('next'))
        self.next_btn.pack(side=tk.LEFT, padx=2)
        self.last_btn = ttk.Button(controls_frame, text="Last >>", command=lambda: self.change_page('last'))
    ...
    def bind_mousewheel(self, widget):
        widget.bind("<MouseWheel>", self.on_mousewheel, add='+'); widget.bind("<Button-4>", self.on_mousewheel, add='+'); widget.bind("<Button-5>", self.on_mousewheel, add='+')
    ...
    def load_history(self):
        self.all_history_items = sorted(self.parent.settings.get(HISTORY_SELECTION_KEY, []), key=lambda x: x.get("timestamp", 0), reverse=True)
        self.current_page = 1
        self.display_page()
    ...
    def display_page(self):
        ...
        page_size = self.items_per_page.get()
        ...
        for s_obj in page_items:
            ...
            lines = s_obj["files"]
            txt = tk.Text(fr, wrap='none', height=min(len(lines), 10) if lines else 1); txt.pack(fill=tk.X, expand=True, pady=2)
    ...
    def update_pagination_controls(self):
        page_size = self.items_per_page.get()
        total_items = len(self.all_history_items)
...
class OutputFilesDialog(tk.Toplevel):
    ...
    # ------------------------------
    def __init__(self, parent):
        super().__init__(); self.parent = parent; self.title("View Outputs")
        self.all_files_meta = []; self.filtered_files_meta = []
        self.current_page = 1; self.items_per_page = tk.IntVar(value=100)
        self.active_loading_filepath = None
        self.search_thread = None; self.search_cancelled = threading.Event()
    ...
    def load_files(self):
        ...
        for f in os.listdir(OUTPUT_DIR):
            ...
            if os.path.isfile(fp):
                try:
                    with open(fp, 'r', encoding='utf-8', errors='replace') as cf: char_count=len(cf.read())
                    self.all_files_meta.append({'name': f, 'mtime': os.path.getmtime(fp), 'chars': char_count, 'path': fp})
                except (OSError,UnicodeDecodeError): continue
        self.all_files_meta.sort(key=lambda x: x['mtime'], reverse=True)
        self.filtered_files_meta = self.all_files_meta[:]
        self.current_page = 1
        self.display_page()
    ...
    def display_page(self):
        ...
        end_index = start_index + page_size
        page_items = self.filtered_files_meta[start_index:end_index]
    ...
    def on_file_select(self, event):
        ...
        filepath = self.tree.selection()[0]
        self.active_loading_filepath = filepath
    ...
    def start_search(self):
        ...
        self.search_thread = threading.Thread(target=self._search_worker, args=(term, self.search_cancelled), daemon=True)
    ...
    def save_current_file(self):
        ...
        threading.Thread(target=self._save_file_worker, args=(self.active_loading_filepath, content), daemon=True).start()
        self.on_close()
    ...
    def _search_worker(self, term, cancel_event):
        results = []
        total = len(self.all_files_meta)
        for i, item in enumerate(self.all_files_meta):
            if cancel_event.is_set(): return
            try:
                content = safe_read_file(item['path'])
                if term in content.lower(): results.append(item)
            except Exception: continue
            progress = (i + 1) / total * 100
            self.dialog_queue.put(('search_progress', progress))
        if not cancel_event.is_set(): self.dialog_queue.put(('search_done', results))
...
class CodePromptGeneratorApp(tk.Tk):
    ...
    def create_top_widgets(self, container):
        ...
        qf = ttk.LabelFrame(container, text="Quick Action", style='TemplateOps.TLabelframe'); qf.pack(side=tk.RIGHT, fill=tk.Y, padx=5)
    ...
    def add_history_selection(self, selection):
        ...
        self.settings[HISTORY_SELECTION_KEY] = sorted(history, key=lambda x: x["timestamp"], reverse=True)[:50] # Increased history size
    ...
    def on_quick_copy_selected(self, _):
        ...
        if val in self.templates and "{{CLIPBOARD}}" in self.templates[val]:
            ...
            return
        op_map = {"Replace \"**\"": lambda t: self._extended_text_cleaning(t), "Gemini Whitespace Fix": lambda t: t.replace('\u00A0', ' '),
                  "Remove Duplicates": lambda t: '\n'.join(dict.fromkeys(t.rstrip('\n').split('\n'))), "Sort Alphabetically": lambda t: '\n'.join(sorted(t.rstrip('\n').split('\n'))),
                  "Sort by Length": lambda t: '\n'.join(sorted(t.rstrip('\n').split('\n'), key=len)), "Escape Text": lambda t: t.rstrip('\n').encode('unicode_escape').decode('ascii', 'ignore'),
    ...
    def _strip_bold_markers_safely(self, text):
        return self._extended_text_cleaning(text)

    def process_quick_copy_format(self, text):
        text = unify_line_endings(text)
        text = self._extended_text_cleaning(text)                                   # strip **, handle lone “>”
        lines = text.split('\n')
        ...
        notification = (
            ...
            if between else
            f"ℹ️ {'Only one' if len(delim_idx) == 1 else 'No'} ‘---’ found – copied whole document ({char_cnt} chars)."
        )
